...
void OpenClicked(GtkButton *obj, gpointer data)
{
	...
	if (resp == GTK_RESPONSE_ACCEPT) {
		...
		music = BASS_MusicLoad(0, file, 0, 0, GetFlags(), 1); // load the new MOD music
		if (music) { // success
			...
			gtk_button_set_label(obj, strrchr(file, '/') + 1);
			{
				...
				snprintf(text, sizeof(text), "name: %s, format: %dch %s%s",
...
int main(int argc, char* argv[])
{
	...
	{ // initialize file selector
		...
		regcomp(&fregex, "\\.(mo3|xm|mod|s3m|it|umx)$", REG_ICASE | REG_NOSUB | REG_EXTENDED);
